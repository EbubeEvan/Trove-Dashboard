...
`localStorage`. The localStorage flag is the lightweight indicator that route guards and the Zustand
auth store read from on app load.
...
- Dark mode toggle in the top bar with persisted preference (localStorage). The dark palette was
...
- Localization for the three major Nigerian languages: Hausa, Yoruba, and Igbo.
